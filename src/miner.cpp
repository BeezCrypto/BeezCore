...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("BeezMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("BeezMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("BeezMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("beez-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running BeezMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
